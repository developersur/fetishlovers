<script type="text/javascript">
    ...
    function posicionarMenu() {
        ...
        var altura = parseFloat(parseFloat(altura_del_header)+parseFloat(altura_del_menu));
        if ($(window).scrollTop() >= altura){
            $('#carrito_cabecera').addClass('fixed');
            $('#carrito_cabecera').css('margin-top', '20px');
        } else {
            $('#carrito_cabecera').removeClass('fixed');
            $('#carrito_cabecera').css('margin-top', '0');
        }
    }
    ...
    $(document).ready(function() {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>index.php/Carro/Mostrar",
        ...
        $(document).on('submit','form.Form_Agregar',function(e){
            ...
            $.ajax({
                ...
                success:function(data){
                    //alert(data);
                    $("#carrito_cabecera").html(data);
                    $("#carrito_cabecera").addClass("open");
                    //alert(data);
                    //$("#carro_de_compras").html(data);
                }
            });
